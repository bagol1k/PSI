...
ggplot(frequent_terms, aes(x = FREQ, y = reorder(WORD, FREQ))) + #posortowane tak ze najczestsze na gorze
  geom_bar(stat = "identity", fill = "skyblue", color = "darkblue", alpha = 0.8) +#alpha-przezroczystosc
  labs(x = "Częstość", y = "Słowo") +
  ggtitle("Wykres częstości słów") +
  theme_minimal() + #usuwa zbedne elementy tla np. szare tlo
  theme(axis.text.y = element_text(size = 10), # Dostosowanie rozmiaru czcionki etykiet na osi Y
        ...
        axis.line = element_line(color = "black")) # Dodaje czarne osie
